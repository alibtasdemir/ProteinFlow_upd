...
from dataset import mmcif_parsing, parsers
...
def _retrieve_mmcif_files(    
    ...
    total_num_files = 0
    all_mmcif_paths = []
    for fpath in tqdm(os.listdir(mmcif_dir)):
        mmcif_file = os.path.join(mmcif_dir, fpath)
        if os.path.isdir(mmcif_file):
        ...
        total_num_files += 1
        if min_file_size <= os.path.getsize(mmcif_file) <= max_file_size:
            all_mmcif_paths.append(mmcif_file)
...
def process_mmcif(
    ...
    processed_mmcif_path = f'{mmcif_name}.pkl'
